...
fun obtainSystemGeneratedPlaylist(id: Int, context: Context): Pair<String, List<Recommendation>> {
    return when (id) {
        ...
        2 -> Pair<String, List<Recommendation>>(context.resources.getString(R.string.songs), obtainFavoriteSongsList())
        else -> Pair<String, List<Recommendation>>(context.resources.getString(R.string.downloaded), obtainDownloadSongsList())
    }
}

fun obtainFavoriteSongsList(): List<Song> {
    // TODO: HACER
...
fun obtainFollowedPlaylists(): List<Playlist> {
    ...
    listaCanciones.add(cancion1)
    listaCanciones.add(cancion2)
    val playlist = Playlist(1, "dfsdfdsfd", autor1, "http://storage.googleapis.com/automotive-media/album_art_2.jpg", listaCanciones)
    devolver.add(playlist)
    return devolver
}

fun obtainDownloadSongsList() : List<Song>{
    // TODO: HACER
    return obtainFavoriteSongsList()
}